                ...
                <div class="col-md-12">
                    <div class="panel panel-flat">
                        <div class="panel-heading">
                            <a href="{{url('/add-sell-buy-place')}}">
                                <span class="pull-right">
                                    <p class="btn btn-info btn-xs"><i class="icon-plus-circle2"></i> اضف مكان للبيع </p>
                                </span>
                            </a>
                            <h6 class="panel-title"><i class="icon-cash2"> </i>  بيع وشراء  </h6>
                            <hr>
                        </div>
                            ...
                            <div class="content">
                                ...
                                <div class="panel panel-flat">
                                    <table class="table datatable-basic">
                                        ...
                                        <tbody>
                                        <?php $i = 0; ?>
                                        @foreach($sell_buy_places as $sell_buy_place)
                                            <tr>
                                                <td>{{ ++$i }}</td>
                                                <td>{{$sell_buy_place->place_name}}</td>
                                                <td>
                                                    @foreach($place_types as $place_type)
                                                        @if($sell_buy_place->place_type == $place_type->id )
                                                            {{$place_type->title}}
                                                        @endif
                                                    @endforeach
                                                </td>
                                                <td>{{$sell_buy_place->place_address}}</td>
                                                <td>{!! $sell_buy_place->place_desc!!}</td>
                                                <td>{{$sell_buy_place->publish_date }}</td>
                                                <td><a href="{{url('/edit-sell-buy-place/'.$sell_buy_place->id)}}"><i class="icon-pencil5"></i> تعديل </a></td>
                                                <td><a href="{{url('/delete-place/'.$sell_buy_place->id)}}" onclick="return confirm('هل تريد حذف هذا المكان ؟');"><i class="icon-trash"></i> حذف </a></td>
                                            </tr>
                                        @endforeach
